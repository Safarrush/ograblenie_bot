...
def process_coefficients(message, selected_event, selected_matches_numbers, coefficients_dict, index=0):
    try:
        ...
        bot.reply_to(
            message, f"Введите коэффициент для бойца {match[0]}. Пример (1.2):")
...
def process_fighter1_coefficient(message, selected_event, selected_matches_numbers, coefficients_dict, index, match):
    try:
        ...
        bot.reply_to(
            message, f"Введите коэффициент для бойца {match[1]}. Пример (1.2):")
        ...
        process_coefficients(
            message, selected_event, selected_matches_numbers, coefficients_dict, index=0)
        ...
        if index < len(selected_matches_numbers):
            ...
            for num in selected_matches_numbers:
                ...
                if (fighter1_name, fighter2_name) not in existing_matches:
                    ...
                    cursor.execute(
                        'SELECT * FROM selected_matches WHERE fighter1_name = ? AND fighter2_name = ?', (fighter1_name, fighter2_name))
                    existing_match = cursor.fetchone()

                    if not existing_match:
                        selected_matches_to_insert.append(
                            (selected_event[0], fighter1_name, fighter2_name, fighter1_coefficient, fighter2_coefficient))
